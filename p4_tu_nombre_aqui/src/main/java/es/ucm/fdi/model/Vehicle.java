...
/**
 * Clase que representa un vehículo de la simulación. Alamcena su velocidad
 * máxima, su velocidad actual, si ha llegado a su destino o no, la carretera en
 * la que se encuentra, la posción que ocupa en dicha carretera, un alista de
 * cruces con el itinerario a seguir, el tiempo que le queda estando averiado,
 * la posicón dentro del itinerario, y los kilómetros recorridos.
 * 
...
public class Vehicle extends SimObject implements Comparable<Vehicle> {
	...
	private int posEnIti = 1; // posEnIti marca la posicion en el itinerario del
								// proximo cruce al que vamos
	protected int km = 0;

	public Vehicle(String id, int velMaxima, List<Junction> route)
		...
		try {
			...
			throw new SimulatorException("The initial road for the vehicle "
					+ id + " doesn't exist ", se);
		}
		itinerario = route;
	}
	...
	/**
	 * Avanza un vehículo en la carretera, disminuyendo su tiempo de avería si
	 * está averiado o avanzando su posición e incrementando sus kilómetros si
	 * no lo está, y añadiéndolo a la cola del cruce correspondiente si ha
	 * llegado al final de la carretera.
	 */
	@Override
	public void avanza() {
		if (tiempoAveria == 0) {
	 ...
	 * 
	 * @param haLlegado
	 *            true si el vehículo ha llegado a su destino, false si no.
	 */
	...
	/**
	 * Mueve el vehículo a la siguiente carretera de su itinerario, actualizando
	 * si ha llegado a su destino en caso de ser así.
	 */
	...
	public void moverASiguienteCarretera() throws SimulatorException {
		...
		if (posEnIti == itinerario.size() - 1) {
			...
			try {
				road = itinerario.get(posEnIti).carreteraUneCruces(
						itinerario.get(posEnIti + 1));
			} catch (SimulatorException se) {
				throw new SimulatorException(
						"Unkown road in the itinerary of the vehicle " + id
								+ " ", se);
			}
	 ...
	 * 
	 * @param n
	 *            pasos de la simulación que debe estra averiado el vehículo.
	 */
	 ...
	 * 
	 * @param vel
	 *            nueva velocidad actual del vehículo.
	 */
	...
	public String toString() {
		String s = "";
		s += "id = " + id + ", pos = " + pos + ", velActual = " + velActual
				+ ", km = " + km;
		return s;
