...
public class MostCrowdedJunction extends JunctionWithTimeSlice {
	...
	public void switchLights() {
		...
		for (int i = 0; i < incoming.size(); ++i) {
			if (!incoming.get(i).equals(verde)
					&& incoming.get(i).cola.size() > max) {
				max = incoming.get(i).cola.size();
				semaforo = i;
			}
		}
		incoming.get(semaforo).light = true;
		((IncomingRoadWithTimeSlice) incoming.get(semaforo)).setTimeSlice(Math
				.max(max / 2, 1));
		((IncomingRoadWithTimeSlice) incoming.get(semaforo))
				.setUsedTimeUnits(0);
	}
