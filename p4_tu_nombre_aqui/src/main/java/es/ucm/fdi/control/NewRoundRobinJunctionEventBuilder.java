...
/**
 * Crador del nuevo evento de cruce circular
 * 
	...
	public Event parse(IniSection sec) throws IllegalArgumentException {
		if (sec.getTag().equals(TAG) && sec.getValue("type").equals("rr")) {
			try {
				...
				return new NewRoundRobinJunctionEvent(time, id, minIntervalo,
						maxIntervalo);
			} catch (NullPointerException npe) {
				throw new IllegalArgumentException(
						"Missing fields in the round robin junction event section ",
						npe);
			}
		} else
			return null;
